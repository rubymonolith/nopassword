...
RSpec.describe "Email Authentication Flow", type: :request do
  ...
  describe "complete authentication flow" do
    it "allows a user to authenticate via email link" do
      ...
      get "/email_authentication/new"
      expect(response).to have_http_status(:ok)
      expect(response.body).to include("Sign in")

      # 2. Submit email address
      post "/email_authentication", params: { nopassword_email_authentication: { email: "user@example.com" } }
      ...
      get "/email_authentication/#{token}"
      expect(response).to have_http_status(:ok)
  ...
  describe "invalid token" do
    it "rejects wrong tokens" do
      ...
      patch "/email_authentication/wrong-token"
  ...
  describe "email validation" do
    it "rejects invalid emails" do
      post "/email_authentication", params: { nopassword_email_authentication: { email: "not-an-email" } }
      expect(response).to have_http_status(:unprocessable_entity)
    end
