...
RSpec.describe Magiclink::Secret, type: :model do
  let(:data) { "brad@example.com" }
  let(:code) { "123456" }
  let(:remaining_attempts) { Magiclink::Secret::DEFAULT_REMAINING_ATTEMPTS }
  let(:secret) { Magiclink::Secret.new(data: data, code: code, remaining_attempts: remaining_attempts) }
  ...
  context "persisted" do
    before { secret.save! }
    subject { Magiclink::Secret.find_by_digest salt: secret.salt, data: data }
