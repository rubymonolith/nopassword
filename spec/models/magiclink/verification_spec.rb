...
RSpec.describe Magiclink::Verification, type: :model do
  let(:email) { "brad@example.com" }
  let(:authentication) { Magiclink::EmailAuthentication.new(email: email) }
  let(:code) { authentication.code }
  let(:salt) { authentication.verification.salt }
  let(:verification) { Magiclink::Verification.new(code: code, salt: salt, data: email) }
