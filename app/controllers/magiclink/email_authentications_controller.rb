class Magiclink::EmailAuthenticationsController < ApplicationController
  ...
  protected
    ...
    def deliver_authentication(authentication)
      Magiclink::EmailAuthenticationMailer.with(authentication: authentication).notification_email.deliver
    end
    ...
    def verification_exceeded_attempts(verification)
      flash[:magiclink_status] =  "The number of times the code can be tried has been exceeded."
      redirect_to url_for(action: :new)
    end
    ...
    def verification_expired(verification)
      flash[:magiclink_status] =  "The code has expired."
      redirect_to url_for(action: :new)
    end
  ...
  private
    def email_authentication_params
      params.require(:magiclink_email_authentication).permit(:email)
    end

    def verification_params
      params.require(:magiclink_verification).permit(:code, :salt, :data)
    end

    def assign_verification
      @verification = Magiclink::Verification.new(verification_params)
    end

    def assign_email_authentication
      @email_authentication = Magiclink::EmailAuthentication.new(email_authentication_params)
    end

    def initialize_and_assign_email_authentication
      @email_authentication = Magiclink::EmailAuthentication.new
    end
end
